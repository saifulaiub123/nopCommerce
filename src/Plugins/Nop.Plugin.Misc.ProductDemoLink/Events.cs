    ...
    IConsumer<OrderRefundedEvent>
{
    ...
    protected async Task SaveCookiesAsync(Order order, ProductDemoLinkSettings googleAnalyticsSettings, Store store)
    {
        ...
        //client_id
        httpContext.Request.Cookies.TryGetValue(ProductDemoLinkDefaults.ClientIdCookiesName, out var clientId);
        await _genericAttributeService.SaveAttributeAsync(order, ProductDemoLinkDefaults.ClientIdAttribute, clientId, store.Id);

        //session_id
        var measurementId = googleAnalyticsSettings.GoogleId.Split('-')[1];
        var sessionCookieKey = $"{ProductDemoLinkDefaults.SessionIdCookiesName}{measurementId}";
        httpContext.Request.Cookies.TryGetValue(sessionCookieKey, out var sessionId);
        await _genericAttributeService.SaveAttributeAsync(order, ProductDemoLinkDefaults.SessionIdAttribute, sessionId, store.Id);
    }
    ...
    protected async Task ProcessOrderEventAsync(Order order, ProductDemoLinkSettings googleAnalyticsSettings, string eventName)
    {
        try
        {
            ...
            {
                ClientId = await _genericAttributeService.GetAttributeAsync<string>(order, ProductDemoLinkDefaults.ClientIdAttribute, store.Id),
            ...
            var gaParams = new Parameters
            {
                ...
                EngagementTime = 100,
                SessionId = await _genericAttributeService.GetAttributeAsync<string>(order, ProductDemoLinkDefaults.SessionIdAttribute, store.Id),
                Value = orderTotal,
                Tax = orderTax,
    ...
    public async Task HandleEventAsync(OrderRefundedEvent eventMessage)
    {
        ...
        if (sendRequest)
            await ProcessOrderEventAsync(order, googleAnalyticsSettings, ProductDemoLinkDefaults.OrderRefundedEventName);
    }
    ...
    public async Task HandleEventAsync(OrderPaidEvent eventMessage)
    {
        ...
        await ProcessOrderEventAsync(order, googleAnalyticsSettings, ProductDemoLinkDefaults.OrderPaidEventName);
    }
