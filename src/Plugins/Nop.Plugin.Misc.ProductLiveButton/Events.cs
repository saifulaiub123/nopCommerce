    ...
    IConsumer<OrderRefundedEvent>
{
    ...
    {
        return await _widgetPluginManager.IsPluginActiveAsync(ProductLiveButtonDefaults.SystemName);
    }

    protected async Task SaveCookiesAsync(Order order, ProductLiveButtonSettings googleAnalyticsSettings, Store store)
    {
        //try to get cookie
        var httpContext = _httpContextAccessor.HttpContext;

        //client_id
        httpContext.Request.Cookies.TryGetValue(ProductLiveButtonDefaults.ClientIdCookiesName, out var clientId);
        await _genericAttributeService.SaveAttributeAsync(order, ProductLiveButtonDefaults.ClientIdAttribute, clientId, store.Id);

        //session_id
        var measurementId = googleAnalyticsSettings.GoogleId.Split('-')[1];
        var sessionCookieKey = $"{ProductLiveButtonDefaults.SessionIdCookiesName}{measurementId}";
        httpContext.Request.Cookies.TryGetValue(sessionCookieKey, out var sessionId);
        await _genericAttributeService.SaveAttributeAsync(order, ProductLiveButtonDefaults.SessionIdAttribute, sessionId, store.Id);
    }

    /// <returns>A task that represents the asynchronous operation</returns>
    protected async Task ProcessOrderEventAsync(Order order, ProductLiveButtonSettings googleAnalyticsSettings, string eventName)
    {
        try
        {
            ...
            {
                ClientId = await _genericAttributeService.GetAttributeAsync<string>(order, ProductLiveButtonDefaults.ClientIdAttribute, store.Id),
            ...
            var gaParams = new Parameters
            {
                ...
                EngagementTime = 100,
                SessionId = await _genericAttributeService.GetAttributeAsync<string>(order, ProductLiveButtonDefaults.SessionIdAttribute, store.Id),
                Value = orderTotal,
                Tax = orderTax,
    ...
    public async Task HandleEventAsync(OrderRefundedEvent eventMessage)
    {
        ...
        //settings per store
        var store = await _storeService.GetStoreByIdAsync(order.StoreId) ?? await _storeContext.GetCurrentStoreAsync();
        var googleAnalyticsSettings = await _settingService.LoadSettingAsync<ProductLiveButtonSettings>(store.Id);
        ...
        if (sendRequest)
            await ProcessOrderEventAsync(order, googleAnalyticsSettings, ProductLiveButtonDefaults.OrderRefundedEventName);
    }
    ...
    public async Task HandleEventAsync(OrderPaidEvent eventMessage)
    {
        ...
        await ProcessOrderEventAsync(order, googleAnalyticsSettings, ProductLiveButtonDefaults.OrderPaidEventName);
    }
